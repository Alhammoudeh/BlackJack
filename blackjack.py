...
class Chips:
    ...
    def __init__(self):
        self.total = int(input("How many chips are you bringing in the table today? "))  # This can be set to a default value or supplied by a user input
        self.bet = 0
...
def take_bet(chips):
    ...
    while True:
        try:
            chips.bet = int(input("Please provide us with a bet value: "))
...
def hit_or_stand(deck,player):
    ...
    while True:
        prompt = input("Would you like to [H]it or S[tand]?: ").lower()
        print(prompt)
...
def player_busts(hand,chips):
     print(f"Sorry, your hand is at {player.value}. It's a bust! You lost {chips.bet} chips.")
     chips.lose_bet()
...
def dealer_busts(hand,chips):
     print(f"Dealer's hand is at {dealer.value}. It's a bust! You win {chips.bet} chips.")
     chips.win_bet()
